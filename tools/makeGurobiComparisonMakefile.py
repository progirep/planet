...
with open("Makefile","w") as outFile:
    ...
    outFile.write("\t@../../tools/sumUserTimes.py .out/*.ytxt > \"performanceComparisons/Gurobi"+sys.argv[1]+"\"\n")

    for a in allRLVFiles:
        outFile.write(".out/"+a+".ytxt:\n")
        outFile.write("\t@mkdir -p .out\n")
        outFile.write("\t@echo \"Processing: "+a+"\"\n")
        outFile.write("\ttime "+scriptpath+"/../src/planetB "+option+" "+a+" > .out/"+a+".ilp 2> .out/"+a+".ytxt \n")
        outFile.write("\t@echo \"Processing Part 2: "+a+"\"\n")
        if sys.argv[1].find("pprox")>0:
            outFile.write("\t@(time timeout 3600 $$GUROBI_HOME/bin/runGurobi.sh .out/"+a+".ilp >> .out/"+a+".ytxt 2>&1) || if [ $$? -eq 124 ]; then echo \"--> "+a+" timed out\"; else false; fi\n")
        else:
            outFile.write("\t@(time timeout 3600 $$GUROBI_HOME/bin/runGurobi.sh .out/"+a+".ilp > .out/"+a+".ytxt 2>&1) || if [ $$? -eq 124 ]; then echo \"--> "+a+" timed out\"; else false; fi\n")
        outFile.write("\tcat .out/"+a+".ytxt | grep \"^Command exited with non-zero status 124\\|^")
        if a.endswith("_SAT.rlv"):
            outFile.write("Optimal solution found\"\n")
        elif a.endswith("_UNSAT.rlv"):
            outFile.write("Infeasible model\|^Model is infeasible or unbounded\"\n")
        else:
